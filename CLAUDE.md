...
- **`Area3D` already has a `gravity` property**, so `@export var gravity` on an Area3D
  subclass is a *parse error*, not a shadowed field. The arrow's droop is called `drop`.
- **You cannot change `monitoring` from inside `area_entered`.** "Function blocked during
  in/out signal" — the addon's `ignore_collisions` flag exists for exactly this, and
  anything else needs `set_deferred`. A projectile that disarms itself on contact hits both
  cases at once.
- **A held weapon's screen size is decided by `grip.z = -arm_r.z`, and `arm_r.y` is the
  channel that destroys it.** Measured with a throwaway harness that posed the rig and
  printed each candidate's screen span: solving the grip's *yaw* so a bow's plane faced
  forward gave 51 x 69 px for a 1.7 m bow, because 40% of its length pointed at the lens.
  Cancelling the ARM's roll with the grip instead stands the weapon upright with its face
  square to the view — 38 x 103 px, and adding torso roll takes it to 53 x 108 because a
  diagonal arc reads as a bow where a vertical line reads as a stick.
- **Sweeping the weapon arm forward hides the weapon behind the character.** The camera is
  behind the character, so "forward" is the one direction it cannot see. A shield pose with
  18 degrees of forward arm yaw measured 0.67 m *further from the camera than the torso* and
  inside its silhouette, i.e. invisible; purely lateral put it 0.1-0.25 m in front and ~90 px
  clear. Corollary: the reach limit (hand 0.52 m from the shoulder) means a weapon can only
  ever be *just* outside the ribs, so half of anything held will overlap the body.
- **A bow draw cannot be made legible from a camera directly behind the character.** The
  draw axis IS the view axis, so retraction, the string and the nocked arrow are all
...
- **`tools/capture.gd` still resolves the charge glow at
  ...
  harness prints "2 effect node(s)" instead of 3 and `hide_effects` shots keep their glow.
  One line, and it belongs to whoever owns that file.
